...
def error_exit(why):
   print(f"-- Failed to install OpenGL dependencies! Reason: {why}")
   sys.exit(1)
...
if not os.path.isdir(sys.argv[1]):
    error_exit(f"no such directory: {sys.argv[1]}")
try:
    print("-- Downloading OpenGL dependencies...")
    gl_path = os.path.join(sys.argv[1], "GL")
    khr_path = os.path.join(sys.argv[1], "KHR")
    os.mkdir(gl_path)
    os.mkdir(khr_path)
